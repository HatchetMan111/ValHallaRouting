...
# Kartenregion: andorra | saarland | nrw | germany | austria | switzerland | dach | custom
...
if [[ -z "${var_tile_region:-}" && -z "${var_tile_urls:-}" ]]; then
  if command -v pveversion >/dev/null 2>&1; then
    var_tile_region=$(msg_menu "Welche Karte soll Valhalla bauen? (Tiles-Bau dauert je nach Größe Minuten bis >1h)" \
      "germany" "Deutschland (~4 GB PBF, ~25 GB Tiles, 30-60 Min)" \
      "nrw" "NRW (~500 MB, ca. 10-20 Min)" \
      "saarland" "Saarland (~50 MB, wenige Minuten)" \
      "austria" "Österreich (~600 MB)" \
      "switzerland" "Schweiz (~500 MB)" \
      "dach" "D-A-CH (3 Dateien, groß!)" \
      "andorra" "Andorra (~8 MB, Test in 2-3 Min)") || var_tile_region="germany"
  else
